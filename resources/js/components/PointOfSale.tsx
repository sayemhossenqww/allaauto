...
class PointOfSale extends Component<Props, State> {
        ...
        httpService
            ...
            .then((response: any) => {
                if (response.data) {
                    ...
                    this.printInvoice(response.data.order.id);
                }
            })
            .finally(() => {
    ...
    handleSearchSubmit = (event: React.FormEvent<HTMLFormElement>): void => {
        ...
        // Check if the barcode is 18 characters long
        if (searchValue.length === 18 && /^\d+$/.test(searchValue)) {
            // Extract values from barcode
            let flag = searchValue.substring(0, 1);
            let itemCode = searchValue.substring(1, 7);
            let weight = parseFloat(searchValue.substring(7, 12)) / 1000; // Convert to kg
            let totalPrice = parseFloat(searchValue.substring(12, 17)) / 100; // Convert to decimal
    
            // Calculate unit price per kg
            let unitPricePerKg = totalPrice / weight;
    
            console.log("Extracted Barcode Details:", { flag, itemCode, weight, totalPrice, unitPricePerKg });
    
            // Search for the product by matching the retail_barcode
            let foundProduct: IProduct | undefined;
            this.state.categories.forEach(category => {
                console.log("Category:", category);
                let product = category.products.find(p => p.retail_sku === itemCode);
                if (product) {
            ...
            if (foundProduct) {
                ...
                let cartItem = this.state.cart.find(p => p.id === foundProduct!.id);
                if (cartItem) {
                    // Update quantity and price
                    cartItem.quantity = (cartItem.quantity ?? 0) + weight;
                    ...
                    let newItem: ICartItem = {
                        ...
                        price: unitPricePerKg, // Set unit price per kg
                        wholesale_price: foundProduct.wholesale_price,
                        retailsale_price: foundProduct.retailsale_price,
                        retail_barcode: foundProduct.retail_barcode,
                        wholesale_barcode: foundProduct.wholesale_barcode || "", // Add missing wholesale_barcode
                        wholesale_sku: foundProduct.wholesale_sku || "", // Add missing wholesale_sku
                        retail_sku: foundProduct.retail_sku || "", // Add missing retail_sku
                        in_stock: foundProduct.in_stock,
                        track_stock: foundProduct.track_stock,
                        continue_selling_when_out_of_stock: foundProduct.continue_selling_when_out_of_stock,
                        quantity: weight,
                        vat_type: this.getAppSettings().vatType,
                        tax_rate: 0,
                        discount: 0
                ...
                // Clear search input
                this.setState({ searchValue: null });
                let searchInput: any = document.getElementById('barcode-input');
                if (searchInput) searchInput.value = '';
            } else {
                toast.error(t('Product not found!', 'لم يتم العثور على المنتج!'));
            }
    
            return;
        }
    
        // If barcode is not 18 digits, fallback to normal search
        let productFound = 0;
        this.state.categories.forEach(category => {
            let product = category.products.find(p =>
